...
def add_pets_from_json(filename, cur, conn):
    ...
    for pet in json_data:
        ...
        # when do we use cur.execute and when can we just write the code?
        ...
        # when species = 'Rabbit \nDROP TABLE' 
        species_id = cur.fetchone()[0] # what does [0] do?
        # print(cur.fetchone()
...
def non_aggressive_pets(aggressiveness, cur, conn):
    ...
    non_aggressive_pets_list = cur.fetchall() # We use fetchall() to get all rows of the select results
